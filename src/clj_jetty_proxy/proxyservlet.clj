...
(defn- client-registration-created
  ...
  (dosync
	(let [registrations (ensure client-regs-ref)
		...
		(println (str " - SERVS FOR: " servs-for-client))
...
(defn- client-registration-removed
  [client-regs-ref client-reg-root client dir-node]
  (println (str "CLIENT REGISTRATION REMOVED: " dir-node))
  (dosync
	(let [registrations (ensure client-regs-ref)
		reg-def (clojure.string/replace-first dir-node
				(str client-reg-root "/") "")
...
(defn- instance-created
  ...
  ;; sleep a while to make sure the server has time to update the data
  (. Thread sleep 100)
  (println (str "INSTANCE CREATED: " file-node))
  (dosync
...
(defn- instance-removed
  ...
  (println (str "INSTANCE removed: " file-node))
  (dosync
...
(defn- lookup-latest
  ...
  (dosync
	(println "LOOKING UP LATEST")
	(let [f-to-data (ensure file-to-data-ref)
		nodes (keys f-to-data)
		prnodes (println (str "LU LA NODES: " nodes))
...
(<MAJOR> <MINOR>)  - Means that any version where <MAJOR> matches, and
  ...
  (dosync
	...
	for-service (filter (fn [item]
				...
				nodes)
	prfor-service (println (str "LOOKUP SERVICES for-service: " for-service))]
...
(defn- url-of
  ...
  (println (str "URL of: " service-instance))
...
(defn- mapper-fn
  ...
  (do
	...
	(println (str "CLIENT ID: " client-id))
	(println (str "CLIENT VERIFIED: " client-verified))
	(println (str "SERVICE NAME: " service))
	(println (str "SERVICE MAJOR: " major))
	(println (str "SERVICE MINOR: " minor))
	(println (str "URI: " uri))
		...
		(let [serv-tree (tz/find-path (:zipper @(:routes @watcher-ref))
			...
			(if-not serv-tree
				...
				(let [selected-service (lookup-service (:instance-to-load-ref @watcher-ref) (:file-to-data-ref @watcher-ref) (:route-root @watcher-ref) service major minor uri)]
					(println (str "SELECTED: " selected-service))
					selected-service)))))))
...
(defn -main
  ...
  (let [server (Server.)
	...
	(println (str "INIT WATCHER: " watcher))
